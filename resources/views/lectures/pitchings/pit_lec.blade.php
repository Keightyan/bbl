        ...
        @foreach ($pitchings as $pitching)
            @if ($loop->iteration <= 6)
                <div class="col-4">
                    ...
                    @php
                        $image = $pitching->images->first();
                    @endphp
                        @if ($image)
                        <a href="{{ route('lectures.show', ['id' => $pitching->id]) }}"><figure class="post_thumbnail"><div class="new_area-img"><img class="thumbnail" src="{{ asset('storage/'.$image->image_path) }}"><p class="new_area-p">NEW！</p></div></figure></a>
                        @else
    ...
    @elseif ($pitchings->currentPage() >= 2)
        @foreach ($pitchings as $pitching)
                <div class="col-4">
                    ...
                    @endphp
                        @if ($image)
                        <a href="{{ route('lectures.show', ['id' => $pitching->id]) }}"><figure class="post_thumbnail"><div class="new_area-img"><img class="thumbnail" src="{{ asset('storage/'.$image->image_path) }}"></div></figure></a>
                        @else
