        ...
        @foreach ($fieldings as $fielding)
            @if ($loop->iteration <= 6)
                <div class="col-4">
                    ...
                    @php
                        $image = $fielding->images->first();
                    @endphp
                        @if ($image)
                        <a href="{{ route('lectures.show', ['id' => $fielding->id]) }}"><figure class="post_thumbnail"><div class="new_area-img"><img class="thumbnail" src="{{ asset('storage/'.$image->image_path) }}"><p class="new_area-p">NEW！</p></div></figure></a>
                        @else
    ...
    @elseif ($fielding->currentPage() >= 2)
        @foreach ($fieldings as $fielding)
                <div class="col-4">
                    ...
                    @endphp
                        @if ($image)
                        <a href="{{ route('lectures.show', ['id' => $fielding->id]) }}"><figure class="post_thumbnail"><div class="new_area-img"><img class="thumbnail" src="{{ asset('storage/'.$image->image_path) }}"></div></figure></a>
                        @else
